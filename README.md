...
- **求人詳細情報をチャンク単位で処理し、各チャンクの後に続行するかユーザーに確認します（`--skip-chunk-confirm` でスキップ可能）。**
- **「納品希望日」などの追加データを `additionalData` として抽出します。**
...
6.  **確認プロンプトのスキップ:**
    `--skip-chunk-confirm` 引数を使用すると、詳細情報取得開始前の確認と、チャンクごとの継続確認の両方がスキップされます。
    ```bash
    # 最初からスクレイピングし、すべての確認をスキップ
    npx ts-node src/index.ts --skip-chunk-confirm

    # ファイルから再開し、すべての確認をスキップ
    npx ts-node src/index.ts --input-file=data/scraped-YYYY-MM-DD_HH-MM-SS.csv --skip-chunk-confirm
    ```
...
*   **通常モード (`--input-file` なし):**
    ...
    2.  詳細取得に進むかどうかを尋ねます（`--skip-chunk-confirm` でスキップ可能）。
    3.  **フェーズ2 ('y' の場合、またはスキップ時):** 収集された概要の詳細情報を取得します（`--max-jobs` を尊重）。`config.chunkSize` 件ごとに一時停止し、次のチャンクの取得を続けるか確認します（`--skip-chunk-confirm` でスキップ可能）。
...
*   **再開モード (`--input-file` 指定時):**
    ...
    3.  これらの欠落項目について詳細取得に進むかどうかを尋ねます（`--skip-chunk-confirm` でスキップ可能）。
    4.  **フェーズ2 ('y' の場合、またはスキップ時):** 欠落項目について*のみ*詳細情報を取得します（*この実行*での `--max-jobs` を尊重）。`config.chunkSize` 件ごとに確認のために一時停止します（`--skip-chunk-confirm` でスキップ可能）。
...
- `baseUrl`: 求人検索のベースURL。 (環境変数 `BASE_URL` で上書き可能)
- `dataDir`: スクレイピングされたデータファイル（JSON、CSV）が保存されるディレクトリ。 (環境変数 `DATA_DIR` で上書き可能)
- `userAgent`: スクレイピング時に使用するユーザーエージェント。 (環境変数 `USER_AGENT` で上書き可能)
- `pageDelayMs`: ページ遷移後の待機時間（ミリ秒）。 (環境変数 `PAGE_DELAY_MS` で上書き可能)
- `maxConcurrency`: (現在は直接使用されていませんが、将来的な並列処理のための設定項目) (環境変数 `MAX_CONCURRENCY` で上書き可能)
- `chunkSize`: ユーザー確認のために一時停止する前に処理する求人詳細情報の数。 (環境変数 `CHUNK_SIZE` で上書き可能、デフォルトは5)
- `defaultSearchKeyword`: (`src/index.ts` 内で定義) `--keyword` 引数が指定されない場合に使用されるキーワード。
- `maxPages`: (`src/index.ts` 内で定義) 通常モードでスクレイピングするリストページの最大数。
- `jobListingSelector`, `nextPageSelector` など: (主に `src/scraper.ts` や `src/services/detailScraper.ts` 内で定義) スクレイピング用CSSセレクター。
- `timeout`: (主に `src/index.ts` や `src/services/detailScraper.ts` 内で定義) ページナビゲーションや要素待機のタイムアウト（ミリ秒）。
